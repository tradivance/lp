...
### ステップ 3.4: プレビューへの登録（重要）
作成したセクションを「個別確認」できるように登録します。
1.  `src/app/sections/[name]/page.tsx` を開き、作成したコンポーネントをインポート＆リストに追加します。
2.  `src/app/playground/page.tsx` も同様に追加します。
→ これにより、`http://.../sections/[セクション名]` でアクセスできるようになります。
...
4.  **Notionへの登録（推奨）**:
    *   デプロイ完了後、各セクションのURL（例: `https://[アプリ名].vercel.app/sections/Hero`）をNotionの管理ページに貼り付けます。
    *   これで、Notionからワンクリックで各セクションの仕上がりを確認できます。
## 6. AIアシスタントとの協業
